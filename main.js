...
async function openReadingMode(nomor, scrollToAyat = null) {
  ...
  try {
    ...
    const fullAudioUrl =
      ...
      surah.audioFull?.["04"] ||
      surah.audioFull?.["06"] ||
      surah.ayat?.[0]?.audio?.["05"] ||
      surah.ayat?.[0]?.audio?.["01"] ||
      surah.ayat?.[0]?.audio?.["02"] ||
      surah.ayat?.[0]?.audio?.["03"] ||
      surah.ayat?.[0]?.audio?.["04"] ||
      surah.ayat?.[0]?.audio?.["06"] ||
      "";
    ...
    if (fullAudioUrl) {
      versesHTML += `
                <div class="surah-audio-card">
                    <div class="surah-audio-title">Audio Surat Lengkap</div>
                    <audio class="surah-audio-player" controls preload="metadata">
...
async function fetchPrayerAPIByCoords(lat, lng) {
  try {
    ...
    const res = await fetch(
      `https://api.aladhan.com/v1/timings/${day}-${month}-${year}?latitude=${lat}&longitude=${lng}&method=20`,
    );
    const data = await res.json();
...
async function fetchPrayerAPIByCity(city) {
  try {
    ...
    prayerTimesContainer.innerHTML =
      "<p>Gagal memuat jadwal. Periksa nama kota.</p>";
  }
}
...
function renderPrayerTimes(timings) {
  ...
  prayerTimesContainer.innerHTML = prayers
    .map(
      (p) => `
...
function updateCountdown() {
  ...
  if (!nextPrayer) {
    const [fajrHours, fajrMins] = currentPrayerData["Fajr"]
      .split(":")
      .map(Number);
    nextPrayer = {
      key: "Fajr",
      name: "Subuh besok",
      timeMinutes: fajrHours * 60 + fajrMins + 1440,
    }; // +24 hours
  }
  ...
  const activeEl = document.getElementById(
    `prayer-${nextPrayer.key === "Fajr" ? "Fajr" : nextPrayer.key}`,
  );
